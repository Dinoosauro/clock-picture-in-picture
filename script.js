...
function generateCanvas(setNewTimeout) {
    if (is59Seconds && new Date().getMilliseconds() < (document.getElementById("quickUpdate").checked ? 900 : 958) && !setNewTimeout) {
    ...
    let date = new Date()
    if (date.getSeconds() === 59 && date.getMilliseconds() > document.getElementById("quickUpdate").checked ? 900 : 958) date = new Date(date.getTime() + 100);
